...
public class UserServiceImpl implements UserService {
    ...
    public ResponseUtil wechatCheck(String sceneId) {
        ...
        if ("scanned".equals(status)) {
            ...
            if (Objects.nonNull(tokenData)) {
                ...
                if (parts.length > 1)
                    data.put("refreshToken", parts[1]);
                if (parts.length > 2)
                    data.put("email", parts[2]);
                if (parts.length > 3)
                    data.put("nickname", parts[3]);
                if (parts.length > 4)
                    data.put("role", parts[4]);
            }
        }
    ...
    public ResponseUtil wechatCallback(String code, String state) {
        ...
        if (Objects.nonNull(boundUser)) {
            // 更新最新的微信昵称和头像
            if (!nickname.isEmpty())
                boundUser.setNickname(nickname);
            if (!avatar.isEmpty())
                boundUser.setAvatar(avatar);

...
public class UserController {
    ...
    public ResponseUtil logout(
            @Parameter(description = "JWT accessToken", required = true) @RequestHeader("Authorization") String token) {
    ...
    public ResponseUtil wechatCheck(
            @Parameter(description = "场景 ID", required = true, example = "uuid-string") @RequestParam String sceneId) {
        return userService.wechatCheck(sceneId);
    }

    @Operation(summary = "微信 OAuth 回调（手机浏览器访问，已隐藏）", description = "微信在用户确认授权后重定向到此 URL（手机浏览器访问），返回 HTML 页面而非 JSON", hidden = true)
